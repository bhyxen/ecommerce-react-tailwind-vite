...
function Card({ category, images, price, title, description }) {
	const { setCartCount, openProductDetails, setProductDetailsShown } =
	...
	const showDetails = () => {
		openProductDetails();
		setProductDetailsShown({ category, images, price, title, description });
	};
	...
	return (
		<div className="p-5">
			...
			<div
				...
				onClick={showDetails}
				tabIndex="0"
				onKeyDown={(event) => event.key === "Enter" && showDetails}
				aria-label="Product Card Details"
			>
				<figure className="flex h-full flex-col border-solid border border-black rounded-md relative hover:outline-8 outline-black outline-8 transition-transform box-border hover:shadow-2xl overflow-hidden cursor-pointer">
					...
					<img
						className="object-top object-cover aspect-square w-full flex-1"
						src={images && images[0]}
						alt="Headphones"
					/>
